...
import com.mooding.admin.system.base.ISysBaseAPI;
...
import com.mooding.admin.utils.SpringContextUtils;
import com.mooding.admin.utils.cache.RedisUtil;
import com.mooding.admin.utils.security.JwtUtil;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        Set<SysMenu> menus =  sysUserService.getUserMenuSet(username);
    ...
    public LoginUserVo checkUserTokenIsEffect(String token) throws AuthenticationException {
        ...
        LoginUserVo loginUser = sysBaseAPI.getUserByName(username) ;
        if (loginUser == null) {
            throw new AuthenticationException("用户不存在!");
        }
        // 判断用户状态
        if (StringUtils.isNotEmpty(loginUser.getStatus())&& !"0".equals(loginUser.getStatus()) ) {
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        String cacheToken = String.valueOf(redisUtil.get(SystemContans.PREFIX_USER_TOKEN + token));
        if (ObjectConvertUtils.isNotEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
                String newAuthorization = JwtUtil.sign(userName, passWord);
                // 设置超时时间
                redisUtil.set(SystemContans.PREFIX_USER_TOKEN + token, newAuthorization);
